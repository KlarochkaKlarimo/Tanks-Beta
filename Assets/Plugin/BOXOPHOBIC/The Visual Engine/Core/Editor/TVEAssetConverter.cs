...
namespace TheVisualEngine
{
    public class TVEAssetConverter : EditorWindow
    {
            ...
            if (unsupportedPrefabCount > 0)
            {
                ...
                if (GUILayout.Button("Create Prefabs", GUILayout.Height(24)))
                {
                    for (int i = 0; i < prefabObjects.Count; i++)
                    {
                        ...
                        {
                            var gameObject = prefabObjects[i].gameObject;
                            var gameObjectInstance = prefabObjects[i].gameObjectInstance;

                            var newPrefab = Instantiate(gameObject);
            ...
            if (presetIndex != 0 && (supportedPrefabCount > 0 || convertedPrefabCount > 0))
            {
                ...
                if (showAdvancedSettings)
                {
                    ...
                    if (outputMeshes == OutputMeshes.Default)
                    {
                        ...
                        sourcePivots = StyledSourcePopup("Pivots", "Pivots storing for grass when multiple grass blades are combined into a single originalMesh. Stored in UV4.XZY.", sourcePivots, SourcePivotsEnum);
                        ...
                        sourceExtraCoord = StyledSourcePopup("Extra UVs", "Detail UVs used for layer blending for bark and props. Stored in UV2.ZW.", sourceExtraCoord, SourceCoordEnum);
                        optionExtraCoord = StyledCoordOptionEnum("Extra UVs", "", sourceExtraCoord, optionExtraCoord);
        ...
        void DrawConversionButtons()
        {
            ...
            if (StyledMiniToggleButton("", "Collect Converted Data", 12, false))
            {
                ...
                for (int i = 0; i < prefabObjects.Count; i++)
                {
                    ...
                    //if (prefabData.status == OOPrefabMode.Converted || prefabData.status == OOPrefabMode.ConvertedMissingBackup)
                    {
                        prefabObject = prefabObjects[i].gameObject;
        ...
        void StyledPrefab(TVEPrefabData prefabData)
        {
            ...
            var iconRect = new Rect(lastRect.width - 5, lastRect.y + 2, 20, 20);
        ...
        void ConvertPrefab()
        {
            ...
            EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Prepare Prefab", 0.0f);
            ...
            EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Create Backup", 0.1f);
            ...
            EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Prepare Assets", 0.2f);
            ...
            EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Convert Materials", 0.4f);
            ...
            if (outputMeshes != OutputMeshes.Off)
            {
                ...
                EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Transform Meshes", 0.5f);
                ...
                EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Convert Meshes", 0.6f);
                ...
                EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Transform Colliders", 0.7f);
                ...
                EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Convert Colliders", 0.8f);
            ...
            EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Save Prefab", 0.9f);
            ...
            EditorUtility.DisplayProgressBar("The Visual Engine", prefabObject.name + ": Finish Conversion", 1.0f);
        ...
        void RevertPrefab(bool deleteConvertedAssets)
        {
            ...
            if (deleteConvertedAssets)
            {
                ...
                {
                    Debug.Log("<b>[The Visual Engine]</b> " + "The converted prefab data cannot be deleted on revert!");
                }
            }
                    ...
                    for (int j = 0; j < meshRenderers[i].sharedMaterials.Length; j++)
                    {
                        ...
                        if (material != null)
                        {
                            if (!attributes.Contains(material.shader.name))
                            {
                                ...
                                if (isShared == true && TVEUtils.HasLabel(path))
                                {
        ...
        void CreatePrefabBackupFile()
        {
            ...
            AssetDatabase.SetLabels(prefabObject, new string[] { });
        ...
        Material CreateMaterialInstance(Material material, string dataName, bool shareCommonAssets)
        {
            ...
            {
                instance = new Material(shaderSubsurface);
            }
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                if (presetLines[i].StartsWith("Shader"))
                {
                    ...
                    if (type == "SHADER_STANDARD")
                    {
                        ...
                        {
                            shaderStandard = Shader.Find(shader);
                        }
                    }
                    else if (type == "SHADER_SUBSURFACE")
                    {
                        ...
                        if (Shader.Find(shader) != null)
                        {
                            shaderSubsurface = Shader.Find(shader);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                ...
                if (useLine)
                {
                    if (presetLines[i].StartsWith("Utility"))
                    {
                        ...
                        //    packerData = OOUtils.CreatePackerData();
                        //    doPacking = true;
                        //}
                                ...
                                for (int s = 0; s < ShaderUtil.GetPropertyCount(shader); s++)
                                {
                                    ...
                                    if (propType == ShaderUtil.ShaderPropertyType.TexEnv)
                                    {
                                        ...
                                        if (texture != null && !texture.name.Contains("Internal"))
                                        {
                                            ...
                                            if (outputCollection)
                                            {
                                                if (TVEUtils.HasLabel(texturePath))
                                                {
        ...
        {
            shaderStandard = Shader.Find("BOXOPHOBIC/The Visual Engine/Geometry/General Standard Lit");
            shaderSubsurface = Shader.Find("BOXOPHOBIC/The Visual Engine/Geometry/General Subsurface Lit");
        }
